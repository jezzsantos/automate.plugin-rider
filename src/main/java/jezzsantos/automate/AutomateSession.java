...
import jezzsantos.automate.settings.ProjectSettingsState;
import jezzsantos.automate.ui.AutomateToolWindow;
import kotlin.Unit;
...
public class AutomateSession {
    @NotNull
    private static final Icon icon = IconLoader.getIcon("/icons/automate.svg", AutomateSession.class);
    @NotNull
    ...
    private void addTelemetry(@NotNull Telemetry telemetry) {
        if (firstMessage) {
            ...
            toolWindow = new AutomateToolWindow(this, dotNetDebugProcess.getProject(), lifetime);
        ...
        if (toolWindow != null)
            toolWindow.addTelemetry(index, telemetry, visible, AppSettingState.getInstance().filterTelemetryMode.getValue() == FilterTelemetryMode.DEFAULT);
    }
    ...
    private void updateFilteredTelemetries() {
        ...
        if (toolWindow != null)
            toolWindow.setTelemetries(telemetries, filteredTelemetries);
    }
